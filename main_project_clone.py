...
from tkinter import Tk, Text, Button, Listbox, END, Label, Menu,Radiobutton
...
def find_advantages_in_text(full_text):
    keywords = ["технический результат", "технической задачей", "позволяет", "достигается", "улучшает", "повышает"]

    text_box.tag_remove("advantages_highlight", "1.0", END)
    ...
    for sent in sentences:
        ...
        if any(keyword in sent_text for keyword in keywords):
            ...
            if start_idx:
                end_idx = f"{start_idx}+{len(sent.text)}c"
                text_box.tag_add("advantages_highlight", start_idx, end_idx)
                highlighted_ranges.append((start_idx, end_idx))

    text_box.tag_config("advantages_highlight", background="green", foreground="black")

# def switch_databases(save_weak,save_adv):
#     if save_weak == "True":
...
def highlight_existing_weaknesses():
    ...
    cursor.execute("SELECT text FROM weaknesses WHERE inventorOrAuthor != 'Виталий Павлович Понкратов'")  
...
def find_RU_patents(patent_text):
    ...
    text_box.tag_remove("patent_highlight", "1.0", END)  

    for match in matches:
        start_idx = "1.0"
        while True:
            start_idx = text_box.search(match, start_idx, stopindex=END)
            if not start_idx:
                break
            end_idx = f"{start_idx}+{len(match)}c"
            text_box.tag_add("patent_highlight", start_idx, end_idx)
            start_idx = end_idx

    text_box.tag_config("patent_highlight", background="pink",foreground="black")
...
def show_context_menu(event, root, patent_text, patent_id):
    m = Menu(root, tearoff=0)

    # Удаляем старое выделение номеров патентов
    text_box.tag_remove("patent_highlight", "1.0", END)
    ...
    m.add_command(label=patent_id,
                      command=lambda p=patent_id: save_to_second_column(selected_text,p)
                      )
    # Если текст выделен, продолжаем
    if not patent_text:
        m.add_command(label="Нет определенных патентов в тексте")
    else:
        ...
        for patent_t in patent_text:
            ...
            m.add_command(
                label=patent_t,
                command=lambda p=patent_t: save_to_second_column(selected_text, p)  # Передаем patent_id
            )
        
        # Выделяем номера патентов в тексте
        for patent_t in patent_text:
            start_idx = "1.0"
            while True:
                start_idx = text_box.search(patent_t, start_idx, stopindex=END)
                if not start_idx:
                    break
                end_idx = f"{start_idx}+{len(patent_t)}c"
                text_box.tag_add("patent_highlight", start_idx, end_idx)
                start_idx = end_idx
...
def show_results():
    global listbox, btn_next, btn_prev, text_box, label_index, patents, current_index, saved_weaknesses_box,label_id
    root = Tk()

    selected_options= tk.StringVar(value="weaknesses")
    # Привязываем контекстное меню к текстовому полю
    root.bind("<ButtonRelease-3>", lambda e: show_context_menu(e, root, find_RU_patents(patents[current_index]["text"]), patents[current_index]["id"]))
    ...
    def load_save_weaknesses():
        if selected_options.get() == "weaknesses":   
            cursor.execute("SELECT text from weaknesses")
        elif selected_options.get() == "advantages":
            cursor.execute("SELECT text from advantages")
        saved_weaknesses_box.delete(0, END)
        for row in cursor.fetchall():
            saved_weaknesses_box.insert(END, row[0])

    save_weak = Radiobutton(text="Сохранить в недостатки",variable=selected_options,value="weaknesses",command=load_save_weaknesses)
    save_weak.grid(row=2,column=2,padx=10,pady=10,sticky="e")
    save_adv = Radiobutton(text="Сохранить в преимуществах",variable=selected_options,value="advantages",command=load_save_weaknesses)
    save_adv.grid(row=2,column=3,padx=10,pady=10,sticky="e")
    ...
    def save_selected_text():
        ...
        try:
            # Разбираем текст на патент, ID и текст
            ...
            #patent_id = match.group(2).strip()
            weakness_text = match.group(2).strip()

            if not patent_number or not weakness_text:
                raise ValueError("Номер патента, ID или текст недостатка пустые")
            ...
            try:
                if selected_options.get() == "weaknesses":
                    ...
                    cursor.execute(
                        "INSERT INTO advantages (text, patent_id) VALUES (?, ?)",
                        (weakness_text, patent_number)
                    )
                conn.commit()
...
def update_text():
    ...
    if patents:
        ...
        label_id.config(text=f"ID патента: {patent_id}")  # Отображаем ID патента

        find_weaknesses_in_text(patent_text)
        find_advantages_in_text(patent_text)
...
if __name__ == "__main__":
    ...
    query_result = client.query("SELECT id, description FROM patent_google WHERE inventorOrAuthor != 'Виталий Павлович Панкратов'")
    patents = [{"id": row[0], "text": row[1]} for row in query_result.result_set if row[1] is not None]
